// Does the model actually OBEY her sombre/professional definition? (2026-08-23)
...
for(const [label,ans] of [['GLAM (the hard case, shops Neiman/Saks)',GLAM],['QUIET',QUIET]]){
  for(const ask of ['something for a funeral','job interview outfit']){
    ...
    console.log(`\n══ ${label} — "${ask}"`);
    console.log('   her definition reached the prompt:', /NEVER anything sexy or revealing/.test(p)?'yes':'NO');
