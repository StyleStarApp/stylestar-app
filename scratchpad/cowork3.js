...
ok('allowed_domains is the server list (100 stores — Kate Spade removed 2026-08-12)', Array.isArray(tool.allowed_domains) && tool.allowed_domains.length === 100, 'got ' + (tool.allowed_domains || []).length);
...
ok('search works with NO search_domains at all (new client protocol)', (res.headers.get('content-type') || '').includes('text/event-stream') && lastUpstream.body.tools[0].allowed_domains.length === 100);
...
for (const [label, junk] of [['a string', 'nope'], ['a URL', ['https://x.com']], ['numbers', [42]], ['150+ entries', Array.from({ length: 200 }, (_, i) => 'a' + i + '.com')]]) {
  ...
  ok('junk client domains (' + label + ') ignored, not a 400', res.status === 200 && lastUpstream && lastUpstream.body.tools[0].allowed_domains.length === 100);
}
...
ok('blocked store pruned and retried', upstreamCalls.length === 2 && upstreamCalls[0].includes('gucci.com') && !upstreamCalls[1].includes('gucci.com') && upstreamCalls[1].length === 99);
...
replyQueue = [SSE];
res = await handler(fnReq({ max_tokens: 600, messages: MSGS, search: true }));
ok('next request skips the blocked store on the FIRST call (memo)', upstreamCalls.length === 1 && !upstreamCalls[0].includes('gucci.com') && upstreamCalls[0].length === 99);
...
ok('memo now holds all three', upstreamCalls[0].length === 97);
...
const links = await page.evaluate(() => {
  ...
  // A URL on ANOTHER of our 100 stores is still a mismatch for the named store.
