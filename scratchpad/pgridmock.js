...
// current
await shot('pgrid-current.png');

// B — hub rows inside the pearl panel
const original = await page.evaluate(() => document.querySelector('#photoPortraitHub .pgrid').outerHTML);
await page.evaluate((rows) => {
  document.querySelector('#photoPortraitHub .pgrid').outerHTML = '<div class="ps-rows">' + rows + '</div>';
}, ROWS);
await page.waitForTimeout(250);
await shot('pgrid-b.png');

// C — the panel dissolved: plain hub section like Shop/Build (reuses .bhub look)
await page.evaluate((rows) => {
  const hub = document.getElementById('photoPortraitHub');
  hub.outerHTML = '<div class="bhub" id="photoPortraitHub" style="margin-top:14px">'
    + '<div class="hlbl">Your Style</div>' + rows + '</div>';
}, ROWS);
await page.waitForTimeout(250);
await shot('pgrid-c.png');
