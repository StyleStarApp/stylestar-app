...
// A — whisper between the sub and the cards
await page.evaluate((tip) => {
  document.getElementById('htTip')?.remove();
  const sub = document.querySelector('#s-shopstyle .ss-shop-sub');
  sub.insertAdjacentHTML('afterend', tip);
}, WHISPER);
await shot('hearttip-a.png');

// B — bordered chip in the same spot
await page.evaluate((tip) => {
  document.getElementById('htTip')?.remove();
  const sub = document.querySelector('#s-shopstyle .ss-shop-sub');
  sub.insertAdjacentHTML('afterend', tip);
}, CHIP);
await shot('hearttip-b.png');

// C — whisper BETWEEN the card rows (after the 2nd card), seen mid-scroll
await page.evaluate((tip) => {
  document.getElementById('htTip')?.remove();
  const cards = document.querySelectorAll('#shopStyleContent .shop-card');
  cards[1].insertAdjacentHTML('afterend', tip.replace('margin:2px auto 12px', 'margin:10px auto;grid-column:1/-1'));
}, WHISPER);
await shot('hearttip-c.png');
