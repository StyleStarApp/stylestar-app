...
const info = await pg.evaluate(() => {
  const rectOf = sel => { const el = document.querySelector(sel); if (!el) return null;
    ...
    return { sel, top: r.top, bottom: r.bottom, left: r.left, width: r.width,
      marginTop: cs.marginTop, marginBottom: cs.marginBottom, font: cs.font, color: cs.color, text: el.textContent.slice(0,40) }; };
  const wrap = document.querySelector('#s-journal .story-wrap');
  const wrapCs = getComputedStyle(wrap);
  return {
    wrap: { width: wrap.getBoundingClientRect().width, padding: wrapCs.padding, maxWidth: wrapCs.maxWidth },
    title: rectOf('#s-journal .story-title'),
    byName: rectOf('#s-journal .jrnl-by-name'),
    byRole: rectOf('#s-journal .jrnl-by-role'),
    byDate: rectOf('#s-journal .jrnl-by-date'),
    byLink: rectOf('#s-journal .jrnl-by-link'),
    firstP: rectOf('#s-journal .jrnl-p'),
  };
});
console.log(JSON.stringify(info, null, 2));
await pg.screenshot({ path: 'scratchpad/artimg/before-390.png', fullPage: false, clip: {x:0,y:0,width:390,height:900} });
