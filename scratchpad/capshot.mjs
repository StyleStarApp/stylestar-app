// Does the capital S sit right beside the stars on the Edit title and the Mall
// sign? Her eye decides; this just puts them side by side at her real width.
...
for(const [name,open_,sel,label] of [
  ['edit',"showDream()",'#s-dream .dc-logo','THE EDIT  —  the corner star beside the title'],
  ['mall',"showShop()",'#s-shop .mall-sign','THE MALL  —  the sign under the awning'],
]){
  ...
  await pg.goto('http://localhost:8962/',{waitUntil:'domcontentloaded'});
  ...
  const box=await pg.evaluate(s=>{const e=document.querySelector(s);const r=e.getBoundingClientRect();
    return {y:Math.max(0,Math.round(r.top+window.scrollY)-26),h:Math.round(r.height)+52,
      says:e.textContent.replace(/\s+/g,' ').trim().slice(0,60)};},sel);
  console.log(name.padEnd(6),JSON.stringify(box.says));
  await pg.screenshot({path:`scratchpad/cap-${name}.png`,clip:{x:0,y:box.y,width:375,height:Math.min(box.h,420)}});
  shots.push([`cap-${name}.png`,label]);
  await ctx.close();
}
...
const D=await pg.evaluate(async imgs=>{
  const W=1125,LAB=94,GAP=16;const ims=[];
...
fs.writeFileSync('scratchpad/cap-check.png',Buffer.from(D.split(',')[1],'base64'));
await b.close();srv.close();console.log('scratchpad/cap-check.png');
