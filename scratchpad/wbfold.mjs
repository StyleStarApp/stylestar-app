...
await new Promise(r=>srv.listen(8960,r));
...
for(const [name,move] of [['now',false],['above',true]]){
  ...
  await pg.goto('http://localhost:8960/',{waitUntil:'domcontentloaded'});
...
const imgs=[['fold-now.png','NOW  —  the whisper falls BELOW the line. She lands and never sees it.'],
            ['fold-above.png','MOVED  —  she sees it the second she lands. Cost: Shop it / Save slip under.']]
...
fs.writeFileSync('scratchpad/wb-fold.png',Buffer.from(D.split(',')[1],'base64'));
await b.close();srv.close();console.log('scratchpad/wb-fold.png');
