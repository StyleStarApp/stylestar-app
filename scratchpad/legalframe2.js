...
const OPTS = [
  { label: 'Current (now built) — flat #E8CE7E', css: '' },
  { label: 'A — flat #EDD98F (bleed colour)', css: FLAT('#EDD98F') },
  { label: 'B — flat #F3E6B8 (palest stop)', css: FLAT('#F3E6B8') },
  { label: 'C — flat #DFB94E (mid gold, for reference)', css: FLAT('#DFB94E') },
...
const out = await cp.evaluate(async ({ shots, W, H, SCALE }) => {
  ...
  x.fillText('FAQ FRAME — the light yellow end, built vs neighbours', PAD, 44);
...
fs.writeFileSync(path.join(ROOT, 'scratchpad', 'legalframe2.png'), Buffer.from(out.split(',')[1], 'base64'));
await cp.close();
console.log('\n-> wrote legalframe2.png (every option proven distinct)');
