...
2. **Outfit-results HUB rework (was task 3, still open)** — whether to restyle the vanity-mirror Styling / curtain Shopping / pearl Portrait rooms on `#s-photo-res`. Explore with her whether it needs it.
3. **App-wide gold-buttons → cream sweep** (Cath's standing ask; partially done — pref-done pair + error Try-again). A holistic pass: flatten remaining metallic-gold / gold-fill buttons to the clean cream style.
4. **WB star-as-button** portrait idea (make the hanging star open the Style Portrait, with a soft glow); **"shopping list"** feature idea (both parked from 2026-07-16).
5. **~July 24: Almira/state response** on "Style Star by Catherine, LLC" name approval + trademark filing — watch for it; when it lands, the money-path advances to EIN → business bank → affiliate applications.
Also: bricks were enlarged to 90×54 (2.25×) at her request (#431) — confirm on-device it landed right.
