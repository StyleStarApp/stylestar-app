...
- ✅ **HER TEN DIMENSION SCORES ARE BLESSED (2026-08-27):** `d:[3,8,10,7,8,2,10,8,9,9]`
  (relaxed 3 · alluring 8 · polish 10 · **classic 7** — her correction, drafted at 6 · **trendy 8** — her correction, drafted at 9 · casual 2 · dressy 10 · fitted 8 · neutral 9 · colorful 9). ⚠️ **Classic and trendy landed within one point of each other (7/8), which is the honest read of a house that is directional but not a trend-chaser** — closer to her own middle ground than either of my two drafted extremes. Drafted against Neiman Marcus/Bergdorf/NET-A-PORTER as anchors, archetype tag **"Glamorous Luxe, Trendsetter"** — worth asking her to re-confirm this label still fits now that trendy has come down; "Trendsetter" reads more emphatic than an 8 does. Known-for: **"directional designer ready-to-wear, statement shoes and bags, cult and emerging luxury labels."** Price tier `$$$$` only. ⚠️ **Sizes are HER CONFIRMED FACT, not a gap: "No petite or tall they have XXS - XL and a few XXL."** `s:[]` when the STORES entry lands — same honest-gap shape as NET-A-PORTER and DVF.
...
- ✅✅ **MYTHERESA IS LIVE IN THE MALL (2026-08-27), her ask — "let's put them into the Mall please."** Filed under **Elevated & Designer**, beside NET-A-PORTER/Olivela/Marissa Collections (the peer group her own dims were scored against, not the shoe/bag/jewelry specialist shelf — flag to her that this was a judgment call, not something she specified). Blurb draws on her own words ("lots of bags, shoes, accessories, jewelry"): *"Directional designer fashion with fantastic bags, shoes and jewelry"* — hers to reword. ⚠️ **The Mall needed no search URL and no `SEARCH_DOMAINS` entry** — `renderMall()` wraps every store through `_affUrl` automatically once the MID exists, so this earns immediately.
- 🚨 **THE ONE THING STILL BLOCKING THE FULL `STORES` ENTRY (the stylist's ability to SUGGEST Mytheresa, as opposed to the Mall just linking to it): the search URL.** Tried from the sandbox first — `curl` reaches mytheresa.com (200), but `/us/en/search?q=` renders client-side: a real term and a gibberish term come back within **1 byte** of each other (10151 vs 10152), the exact "cannot verify from here" signature seen on Nordstrom, Olivela and others. **She said "I will send url" — still awaited.** The moment that lands, add the full `STORES` entry (dims + sizes already locked) + `mytheresa.com` to `SEARCH_DOMAINS` in `netlify/functions/style-ai.js` in one commit.
