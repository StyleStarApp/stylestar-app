...
- ✅ **Welcome Back pendant star (`#s-wb .wb-starsvg`) polish** (Cath drove it live via renders):
  - **Removed the gray/silver outline** (`stroke:#B7BCC2` → `stroke:none`) so it's just the soft-yellow star. Applied ONLY to the static pendant star. (Briefly also applied to the WB opening-animation star `body.ss-wb-star .ss-star-svg path` line ~1122, then **Cath said leave the opening animation unchanged → reverted it back to its original gray outline**. So: static pendant = outline-free; opening animation = untouched/original.)
...
4. **Welcome Back top-section redesign** — parked mid-project; she may or may not return to it (star-as-button idea is OFF).
