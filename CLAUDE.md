...
### 👜✅ THE EDIT'S FIRST MYTHERESA PIECE, NOW PHOTOGRAPHED — Gucci GG Canvas Mini Shoulder Bag, $1,100
Her pick, added 2026-08-27 the same day Mytheresa joined `STORES` and the Mall. **Price confirmed regular (not sale) by her directly.** Her note, verbatim: *"Unmistakable horsebit hardware and the Gucci red & green stripe. Perfect touch of luxury."* Link: `mytheresa.com/us/en/women/gucci-gg-canvas-mini-shoulder-bag-beige-p01192311` (verified live, 200).
- ✅ **SHE SENT THE REAL PHOTO** (`mytheresa.com/image/2310/2612/100/0b/P01192311.jpg`, their own CDN, not bot-walled the way the search page is). Source is 2160×2441 (ratio 0.885); the actual 3:4 crop was rendered with Pillow before shipping — centers cleanly, full bag and full strap loop stay in frame, no `pxPos` override needed.
- ✅ **MOVED UP into the photographed cluster** (right after the Etsy bracelet), per the standing "photographed items lead" reordering rule — it had briefly gone in text-only, at the bottom, before she sent the photo.
- ✅ **ADDED TO `WEEK_STARS` TOO**, since she called it *"the perfect one for Star of the week"* — photo, price and her note (91 chars, already inside the one-liner range, nothing to trim) are all in. **⚠️ NOT YET IN THE ROTATION — her call needed:** pin it as the CURRENT Star right now (`WEEK_STAR_PIN`, overriding whatever's showing today), or append it to the queue's end the way the Etsy bracelet was (safe — nothing currently scheduled would move)? Ask before doing either.
- 🚨 **A MECHANICAL LESSON FOR FUTURE SESSIONS, not her problem: inserting a new Edit item between two EXISTING ones is a classic off-by-one trap.** The neighboring item's own `<a class="dc-item-btn">` link sits AFTER its note div and BEFORE its closing `</div>` — an Edit `old_string` that stops at the note div (to insert "after this item") will silently orphan that neighbor's own link into the new block if not careful. Hit once (Tommy Hilfiger's own Nordstrom link got orphaned this way), caught and fixed the same session via a div-balance + dc-item-count-equals-link-count check. **Always re-run that same two-number check after any hand-edit near `.dc-item` blocks** — currently 28 items, 28 links, 10 photographed, divs balanced.
