...
- ⚠️ **A SECOND, SEPARATE SIZE COST WORTH KNOWING: `index.html` is ~900 KB (~224,000 tokens) on its
  own**, because the whole app is one file by design. Reading it is a context window by itself. That
  is not fixable by archiving and is not a problem to solve — it is simply the other half of why
