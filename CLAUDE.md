...
Your Wardrobe is in great shape — the checklist review is DONE (98 items, 10 categories), the list is a **wishlist** (one heart per row, "Ideas →" on every row), the top section is tight, and the hearts are Cath's own shape. All live (PRs #594–#600). Nothing broken.
1. **Small Wardrobe leftovers:** swap the **🛍 emoji on the shop CTA for line art** (do it with the icon pass); the **four long item names** that still wrap at ~360px; **PARKED at Cath's request — "remind me later": let the how-to paragraph step back after first use** (full on first visit, one line like "Tap the ♡ to add · tap Ideas to shop" once she's hearted a few — reclaims ~100px on every later visit).
