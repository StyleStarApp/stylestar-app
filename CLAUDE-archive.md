# Style Star — project archive

The session-by-session build history, moved out of `CLAUDE.md` on 2026-07-28 to keep
the working file readable. **Nothing here was deleted, only moved**, and it is all
still searchable in this file and in git history.

Read this when you need the story of HOW something came to be: a design decision that
was tried and rejected, why a screen looks the way it does, or the detail of a build.
The standing rules, current decisions, store system and open threads all live in
`CLAUDE.md`, which stays the file to read first.
